...
- **L'action branchée est « Annonce Appel Direct » (`VzR7CN_-s-EcEIfG8r1E`)** depuis le 2026-08-14, en remplacement de « Contact » (`M9shCIS00d8cEIfG8r1E`). Sa source est « Clics sur des numéros de téléphone » et elle est rattachée à l'objectif « Leads par téléphone » — c'est précisément ce que mesure le tracker, là où « Contact » était un « Site Web » générique. Aucune conversion n'a été perdue : « Contact » n'en avait jamais enregistré. **« Contact » ne se déclenche donc plus** ; elle reste « principale » dans le compte, à archiver ou passer en secondaire côté Ads.
...
- [x] ~~Brancher Google Search Console~~ — **la propriété existe**, validée par DNS : `doogaventure.fr` porte un TXT `google-site-verification=Dh5gFWzlwksnAmY8hZdwI2xYgZyVBjtx73HeJIFDjyk` (constaté le 2026-08-13). Validation par DNS = propriété de type « Domaine », qui couvre le `www` et tous les sous-domaines. L'ancien domaine a la sienne, différente.
  - Reste à confirmer dans l'interface que `/sitemap.xml` y a bien été **soumis** — c'est une étape distincte de la validation. Enjeu faible : `robots.txt` déclare déjà le sitemap et le site a été indexé en 24 h.
